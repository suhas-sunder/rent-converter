...
- Pages use fixed-point bigint money parsing (parseMoneyInputToScaled) and Intl.NumberFormat for display.
- Results must hide on invalid/ambiguous input (no misleading 0 outputs).
- Route slugs must stay consistent everywhere (meta canonical/og:url/schema/whitelist/links). Do not invent routes.
- Keep the “How it works” section above the FAQ.
- Preserve decimals end-to-end for calculations; rounding is display-only.

Required fixes (apply to this page and mirror across similar pages)

1. Currency default: USD

- Default currency state to "USD" on SSR and first load.
- When reading localStorage, validate using isCurrency(). If invalid/missing, fall back to "USD" (not CAD).
- Ensure the currency <select> fallback also uses "USD".

2. Comma grouping for large numeric inputs without breaking calculations
...
3. Display decimals: consistent behavior with default 2
...
4. Currency conversion correctness for all supported currencies (site-wide contract)
...
Deliverables
A) List the exact minimal edits you will make on THIS page (bulleted).
B) Output the updated code for the affected parts (full functions/blocks you touched, not pseudocode).
C) Keep changes minimal. No large refactors.

There may be code related to downloading/exporting CSV format. I removed that feature. You can remove it, but do it carefully without breaking any other features. You can also remove and text that mentions csv download or export.

There's a list of valid links available for internal linking. If it's worth linking more pages, feel free to do so with the appropriate contextual information. Don't overdo it if there are already enough links being displayed.

Page provided for inspection (apply fixes here):

- rent-as-percentage-of-income-calculator (code pasted below)
